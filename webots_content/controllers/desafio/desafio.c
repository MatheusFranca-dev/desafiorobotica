...
// maximal stop light value
#define MAX_LIGHT 712
...
int main() {
  ...
  WbDeviceTag light_sensor_0;
  light_sensor_0 = wb_robot_get_device("ls0");
  wb_light_sensor_enable(light_sensor_0, time_step);
  ...
  double distance, speed_modifier, sensor_value, ls0_value;
  ...
  while (wb_robot_step(time_step) != -1) {    
    ...
    ls0_value = wb_light_sensor_get_value(light_sensor_0);
    ...
    switch (state) {
      ...
      case LEFT:
        ...
        else if (wheel_weight_total[0] > WHEEL_WEIGHT_THRESHOLD || wheel_weight_total[1] > WHEEL_WEIGHT_THRESHOLD) {
          speed[0] = 0.9 * MAX_SPEED;
          speed[1] = -0.9 * MAX_SPEED;
        } else {
          speed[0] = MAX_SPEED;
